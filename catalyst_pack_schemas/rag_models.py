...
class QdrantConfig:
    ...
    qdrant_url: str = field(
        default="http://localhost:6333", metadata={"description": "Qdrant server URL"}
    )
    qdrant_api_key: Optional[str] = field(
        default=None, metadata={"description": "Qdrant API key for authentication"}
    )
    collection_name: str = field(
        default="documents", metadata={"description": "Qdrant collection name for storing vectors"}
    )
    ...
    colpali_device: str = field(
        default="cuda", metadata={"description": "Device for ColPali model (cuda/cpu)"}
    )
    ...
    enable_two_stage_retrieval: bool = field(
        default=True, metadata={"description": "Enable Qdrant's 13x performance optimization"}
    )

    # Vector settings
    vector_size: int = field(default=128, metadata={"description": "Vector dimension size"})
    distance_metric: VectorDistanceMetric = field(
        default=VectorDistanceMetric.COSINE, metadata={"description": "Vector similarity metric"}
    )

    # Processing settings
    batch_size: int = field(
        default=32, metadata={"description": "Batch size for document processing"}
    )
    max_retries: int = field(
        default=3, metadata={"description": "Maximum retry attempts for API calls"}
    )
    timeout_seconds: int = field(default=60, metadata={"description": "Request timeout in seconds"})

    # Storage optimization
    on_disk_payload: bool = field(
        default=False, metadata={"description": "Store payload on disk to save RAM"}
    )
...
class RAGConfiguration:
    ...
    # Basic settings
    enabled: bool = field(
        default=False, metadata={"description": "Enable RAG functionality for this pack"}
    )
    provider: RAGProvider = field(
        default=RAGProvider.QDRANT, metadata={"description": "RAG/Vector database provider"}
    )

    # Provider-specific configurations
    qdrant_config: Optional[QdrantConfig] = field(
        default=None, metadata={"description": "Qdrant-specific configuration"}
    )
    ...
    auto_index: bool = field(
        default=False, metadata={"description": "Automatically index new documents"}
    )
    chunk_size: int = field(
        default=1000, metadata={"description": "Document chunk size for text documents"}
    )
    chunk_overlap: int = field(
        default=200, metadata={"description": "Overlap between document chunks"}
    )

    # Search settings
    default_top_k: int = field(
        default=5, metadata={"description": "Default number of results to return"}
    )
    score_threshold: float = field(
        default=0.7, metadata={"description": "Minimum similarity score for results"}
    )

    # Cost management
    enable_cost_tracking: bool = field(
        default=True, metadata={"description": "Track RAG processing costs"}
    )
    monthly_budget_usd: Optional[float] = field(
        default=None, metadata={"description": "Monthly budget limit in USD"}
    )

    # Access control (simplified for MVP)
    access_level: Literal["global", "user", "organization"] = field(
        default="global", metadata={"description": "Document access level (global for MVP)"}
    )
...
class RAGToolConfig:
    ...
    rag_enabled: bool = field(
        default=False, metadata={"description": "Tool uses RAG for enhanced responses"}
    )
    search_scope: Literal["global", "user", "all"] = field(
        default="global", metadata={"description": "Search scope for RAG queries"}
    )
    include_metadata: bool = field(
        default=True, metadata={"description": "Include document metadata in results"}
    )
    max_results: int = field(default=5, metadata={"description": "Maximum results to return"})
